...
import { Send, LogOut, MessageSquare, PlusCircle, Trash2, Edit2, Check, X } from "lucide-react";
...
import 'katex/dist/katex.min.css'; // This is what makes the math look beautiful!
...
export default function SocratiQChat() {
  ...
  // --- NEW: RENAME AND DELETE FUNCTIONS ---
  const deleteChat = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Prevents loading the chat when you click delete
  ...
  if (!session) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-950 text-white">
        ...
        <p className="mb-8 text-gray-400">Please log in to start learning.</p>
        <Link href="/login" className="bg-blue-600 px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition">
          Go to Login
        </Link>
      </div>
    );
    ...
    <div className="flex h-screen bg-gray-950 text-white">
      ...
      <div className="w-64 bg-gray-900 border-r border-gray-800 flex flex-col p-4">
        ...
        <button 
          ...
          className="w-full flex items-center gap-2 bg-blue-600/10 text-blue-500 hover:bg-blue-600/20 py-2 px-3 rounded-lg mb-6 transition"
        >
                ...
                {editingId === chat.id ? (
                  <div className="flex w-full items-center gap-2 p-2">
                    <input 
                      ...
                      className="w-full bg-gray-950 text-sm text-white border border-gray-700 rounded px-2 py-1 focus:outline-none"
                    />
      ...
      <div className="flex-1 flex flex-col">
            ...
            <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] p-4 rounded-2xl ${m.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-800 border border-gray-700 text-gray-200'}`}>
                {m.role === 'user' ? (
                  ...
                  /* NEW: MARKDOWN AND MATH RENDERING (Fixed for v9) */
        ...
        <form onSubmit={handleSubmit} className="p-6 bg-gray-950">
          <div className="max-w-3xl mx-auto relative">
            ...
            />
            <button type="submit" disabled={isLoading} className="absolute right-3 top-3 p-2 bg-blue-600 rounded-lg hover:bg-blue-700 transition disabled:opacity-50">
              <Send size={18} />
            </button>
          </div>
        </form>
